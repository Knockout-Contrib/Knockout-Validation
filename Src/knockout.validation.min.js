...
null; return !0
    ...
    }; ko.validation.rules.phoneUS = { validator: function (a) { if ("string" !== typeof a) return !1; a = a.replace(/\s+/g, ""); return 9 < a.length && a.match(/^(1-?)?(\([2-9]\d{2}\)|[2-9]\d{2})-?[2-9]\d{2}-?\d{4}$/) }, message: "Please specify a valid phone number" }; ko.validatedObservable = function (a) {
        if (!ko.validation.utils.isObject(a)) return ko.observable(a).extend({ validatable: !0 }); var b = ko.observable(a); b.errors = ko.validation.group(a);
        b.isValid = ko.dependentObservable(function () { return 0 === b.errors().length }); return b
    } 
})();